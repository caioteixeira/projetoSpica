...
public class LevelLoader : MonoBehaviour
{
	...
	IEnumerator Load ()
	{
		...
		UniqueIdentifier.AllIdentifiers = UniqueIdentifier.AllIdentifiers.Where(a=>a!=null).ToList();
		var created = new List<LevelSerializer.StoredItem>();

				//First step is to remove any items that should not exist according to the saved scene
		foreach (var go in UniqueIdentifier.AllIdentifiers.Where(a=>a != null)
				.Where(n=>!Data.StoredObjectNames.Any(sn=>sn.Name == n.Id)).ToList())
		{
		...
		//using (new Radical.Logging())
		{
			...
			using (new UnitySerializer.SerializationScope())
			{
				...
				foreach (var item in Data.StoredItems.GroupBy(i=>i.Name,(name, cps)=>
					new {
					  Name = name,
					  Components = cps.Where(cp=>cp.Name==name).GroupBy(cp=>cp.Type, (type, components)=>new { Type = type, List = components.ToList() } )
				}))
				{
					Radical.Log ("\n*****************\n{0}\n********START**********\n", item.Name);
					Radical.IndentLog ();
